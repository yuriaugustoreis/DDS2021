...
--
-- Host: 127.0.0.1
-- Tempo de geração: 03/07/2021 às 22:32
...
--
-- Estrutura stand-in para view `consulta_dados`
-- (Veja abaixo para a visão atual)
--
CREATE TABLE `consulta_dados` (
`Nome do Funcionário` varchar(46)
,`Email` varchar(25)
,`Salário` float(22,2)
,`Contratação` date
...
(290, 'Teste_Departamento', 200, 1700);
...
('TST_TESTE', 'Teste_Programação', 10000.00, 100000.00);
...
(3005992, 'Yuri', 'Augusto dos Reis', 'yuri@teste.com.br', '997792203', '2010-08-22', 'AD_VP', 1800.00, 0, 100, 90);
...
(1090, '917 Avenida Teste', '14811240', 'Araraquara', 'São Paulo', 'TS'),
...
--
-- Estrutura para view `consulta_dados`
--
DROP TABLE IF EXISTS `consulta_dados`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `consulta_dados`  AS SELECT concat(`funcionario`.`NOME`,' ',`funcionario`.`SOBRENOME`) AS `Nome do Funcionário`, `funcionario`.`EMAIL` AS `Email`, `funcionario`.`SALARIO` AS `Salário`, `funcionario`.`DATA_CONTRATACAO` AS `Contratação`, `departamento`.`NOME_DEPARTAMENTO` AS `Departamento`, `localizacao`.`CIDADE` AS `Cidade`, `localizacao`.`ENDERECO` AS `Endereço`, `pais`.`NOME_PAIS` AS `País` FROM ((((`funcionario` join `departamento` on(`funcionario`.`ID_DEPARTAMENTO` = `departamento`.`ID_DEPARTAMENTO`)) join `funcao` on(`funcionario`.`ID_FUNCAO` = `funcao`.`ID_FUNCAO`)) join `localizacao` on(`departamento`.`ID_LOCALIZACAO` = `localizacao`.`ID_LOCALIZACAO`)) join `pais` on(`localizacao`.`ID_PAIS` = `pais`.`ID_PAIS`)) ;
